...
void GameScene7::Update(float dTime) {
	Scene::Update(dTime);
	UISetting();
	TimerSetting(dTime);
	OptimizationObject();
...
void GameScene7::TimerSetting(float dTime) {
	/*coinInstDelayTimer += dTime;*/
	/*obstacleInstDelayTimer += dTime;*/
}

void GameScene7::UISetting() {
...
void GameScene7::OptimizationObject() {
...
void GameScene7::CollisionCheck() {
	/*for (auto iter = coinList.begin(); iter != coinList.end(); iter++) {
	...
	if (enemy->IsCollisionRect((wwater1))&&!(enemy->getColor()==wwater1->getColor())) {
		enemy->changeWater(false);
		wwater1->changeColor(enemy->getColor());
	}
	if (enemy1->IsCollisionRect((wwater1)) && !(enemy1->getColor() == wwater1->getColor())) {
			enemy1->changeWater(false);
	...
	if (player->IsCollisionRect((obstacle))) {
		sceneManager->ChangeScene(new GameScene8());
		return;
	}
	/*}*/
}

void GameScene7::EnemyScale() {
...
void GameScene7::EnemyCheck(float dTime) {
...
void GameScene7::InstantiateObject() {
