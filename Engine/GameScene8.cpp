...
void GameScene8::Update(float dTime) {
	Scene::Update(dTime);
	UISetting();
	TimerSetting(dTime);
	OptimizationObject();
...
void GameScene8::TimerSetting(float dTime) {
	/*coinInstDelayTimer += dTime;*/
	/*obstacleInstDelayTimer += dTime;*/
}

void GameScene8::UISetting() {
...
void GameScene8::OptimizationObject() {
...
void GameScene8::CollisionCheck() {
	/*for (auto iter = coinList.begin(); iter != coinList.end(); iter++) {
	...
	if (enemy->IsCollisionRect((wwater1)) && !(enemy->getColor() == wwater1->getColor())&&enemy->getWater()) {
		enemy->changeWater(false);
...
void GameScene8::EnemyCheck(float dTime) {
...
void GameScene8::InstantiateObject() {
